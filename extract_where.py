...
import sqlparse
from sqlparse.sql import IdentifierList, Identifier, Token, Where, TokenList, Comparison
...
def extract_filter_identifiers(token_stream):
    ...
    for item in token_stream:
        if isinstance(item, Comparison):
            if not isinstance(item.left,Identifier) or not isinstance(item.right,Identifier):
                yield(item.value)
